...
mat adjusted_ps 	= J(`number_dep_vars',1,.)
mat estimated_stars	= J(`number_dep_vars',2,.)
mat reg_pct_control = J(`number_dep_vars',1,.)
mat adjusted_ps_syms = J(`number_dep_vars',1,0)
...
foreach y in `dep_vars'{ 
	...
	* Use p-value to make stars ********************************************
		if (`p' < .1) 	mat estimated_stars[`y_counter',2] = 1 // less than 10%?
		if (`p' < .05) 	mat estimated_stars[`y_counter',2] = 2 // less than 5%?
		if (`p' < .01) 	mat estimated_stars[`y_counter',2] = 3 // less than 1%?
